...
GPolyLineCurve2D *gPolyCurve = NULL;
GReal gDeviation;
...
void BuildFlatContour(const GPolyLineCurve2D* Curve) {
...
void Draw(const GPolyLineCurve2D* Curve) {
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Polyline curve example - Press F1 for help", 640, 480, 16, fullscreen))
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (keys[VK_SPACE]) {
				...
				gPolyCurve->IntersectRay(gIntersectionRay, gIntersectionPoints);
			}
		}
	}
	// Shutdown
