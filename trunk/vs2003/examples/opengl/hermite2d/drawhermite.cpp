...
GHermiteCurve2D *gHermCurve = NULL;
GReal gDeviation;
...
void Draw(const GHermiteCurve2D* Curve) {
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Hermite curve example - Press F1 for help", 640, 480, 16, fullscreen))
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (keys[VK_SPACE]) {
				...
				gHermCurve->IntersectRay(gIntersectionRay, gIntersectionPoints);
			}
		}
	}
	// Shutdown
