...
void BuildFlatContour(const GPath2D* Path) {
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Path example - Press F1 for help", 640, 480, 16, fullscreen))
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (keys[VK_SPACE]) {
				...
				gPath->IntersectRay(gIntersectionRay, gIntersectionPoints);
			}
		}
	}
	// Shutdown
