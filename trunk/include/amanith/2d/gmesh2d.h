...
namespace Amanith {
	...
	*/
	...
	public:
		...
		const GPoint2& Position() const {
			return gPosition;
		}
		//! Set vertex position
		void SetPosition(const GPoint2& NewPos) {
			gPosition = NewPos;
		}
		/*!
	...
	public:
		...
		GMeshVertexEdgeIterator2D(GMeshVertex2D *Vertex) {
...
\endcode
	...
	*/
	...
	public:
		...
		/*!
			...
			of the edge.
		*/
		GBool IsOnEdge(const GPoint2& P, const GReal Tolerance = G_EPSILON);
	};
	...
	*/
	...
	public:
		...
		GBool HasVertex(const GMeshVertex2D *Vertex);
		//! Return G_TRUE if the face includes all the vertexes specified, G_FALSE otherwise.
		GBool HasVertices(const GDynArray<GMeshVertex2D *> VertsArray);
	};
    ...
    public:
		...
		GMeshFaceEdgeIterator2D(GMeshFace2D *Face) {
		...
		GMeshEdge2D *Next() {
			...
			GMeshEdge2D *next = current->Lnext();
	...
	*/
	class G_EXPORT GMesh2D : public GElement {

		friend class GMeshVertexIterator2D;
		friend class GMeshFaceIterator2D;

	private:
		//! The vertexes in this mesh
		GDynArray<GMeshVertex2D *> gVertices;
		//! The faces in this mesh
		GDynArray<GMeshFace2D *> gFaces;
		//! The (quad)edges in the mesh
		GDynArray<GQuadEdge2D *> gQuadEdges;
		/*!
		...
		*/
		GMeshEdge2D *OrbitLeft(GMeshEdge2D *Edge, GMeshFace2D *Left);
		/*!
		...
		*/
		void SetOrbitLeft(GMeshEdge2D *Edge, GMeshFace2D *Left);
		//! Release the storage occupied by a given edge
		void KillEdge(GMeshEdge2D *Edge);
		/*!
		...
		*/
		GMeshEdge2D* DelaunayLocate(const GPoint2& P, GMeshEdge2D *StartEdge);
		/*!
		...
		*/
		GMeshEdge2D *DelaunayInsertSite(const GPoint2& P, const GReal Tolerance, GMeshEdge2D *LastIndertedEdge);
		//! Essentially turns edge e counterclockwise inside its enclosing quadrilateral
		void DelaunaySwap(GMeshEdge2D* e);
	...
	public:
		...
		void DetachEdge(GMeshEdge2D *Edge);
		/*!
		...
		*/
		GMeshEdge2D *OrbitOrg(GMeshEdge2D *Edge, GMeshVertex2D *Org);
		/*!
		...
		*/
		void SetOrbitOrg(GMeshEdge2D *Edge, GMeshVertex2D *Org);
		...
		*/
		static void Splice(GMeshEdge2D *a, GMeshEdge2D *b);
	...
	public:
		...
		GError BuildFromFaces(const GDynArray<GPoint2>& Points, const GDynArray<GIndexList>& FacesIndexes,
							  const GBool Deep2ManifoldCheck = G_TRUE);
		//! Build an incremental Delaunay triangulation.
		GError BuildFromPointsCloud(const GDynArray<GPoint2>& Points, const GReal Epsilon = 2 * G_EPSILON);
		//! Destructor, it release the storage occupied by the contents of this mesh.
		~GMesh2D();
		//! Add a new sub manifold.
		GMeshEdge2D *AddSubManifold();
		/*!
		...
		*/
		GMeshEdge2D *MakeVertexEdge(GMeshVertex2D *Vertex, GMeshFace2D *Left, GMeshFace2D *Right);
		//! Delete a given edge from this mesh, along with its destination vertex (must be non NULL).
		void KillVertexEdge(GMeshEdge2D *Edge);
		/*!
		...
		*/
		GMeshEdge2D *MakeFaceEdge(GMeshFace2D *Face, GMeshVertex2D *Org, GMeshVertex2D *Dest);
		//! Delete a given edge from this mesh, along with its right face (must be non NULL)
		void KillFaceEdge(GMeshEdge2D *Edge);
		...
		GMeshFace2D* Face(const GUInt32 Index);
		...
		void RemoveFace(GMeshFace2D *Face);
		/*!
		...
		*/
		void RemoveVertex(GMeshVertex2D *Vertex);
		//! Find a mesh vertex with the corresponding geometric coordinates.
		GMeshVertex2D *FindVertex(const GReal X, const GReal Y);
		/*!
		...
		*/
		GMeshEdge2D* Connect(GMeshEdge2D* a, GMeshEdge2D* b);
		/*!
		...
		*/
		static GBool IsConnected(GMeshVertex2D *Vert1, GMeshVertex2D *Vert2, GMeshFace2D *Left);
		/*!
		...
		*/
		static GMeshFace2D *RightFace(GMeshVertex2D *Vertex, GMeshFace2D *Left);
	...
	*/
	...
	// Enumerates the vertexes of a given cell in arbitrary order.
	class GMeshVertexIterator2D {
	private:
	...
	public:
		...
		GMeshVertexIterator2D(GMesh2D *Mesh) {
			gMesh  = Mesh;
	...
	public:
		...
		GMeshFaceIterator2D(GMesh2D *Mesh) {
			gMesh  = Mesh;
