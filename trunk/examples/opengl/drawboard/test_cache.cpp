...
void QGLWidgetTest::TestCache(const GUInt32 TestIndex) {
	...
	if (idx == 0) {
		...
		gDrawBoard->InvalidateCacheSlot(gCacheSlot);
		gDrawBoard->SetCacheSlot(gCacheSlot);
		...
		gDrawBoard->SetCachingEnabled(G_TRUE);
		// disable writing on color buffer
		gDrawBoard->SetCachingWriteOnTarget(G_FALSE);
		...
		gDrawBoard->DrawCacheSlot();

		// draw on cache slot and also on screen
		gDrawBoard->SetCachingWriteOnTarget(G_TRUE);
		...
		// draw cached entries [4..7]
		gDrawBoard->DrawCacheEntries(4, 7);
