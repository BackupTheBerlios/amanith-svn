/****************************************************************************
** $file: amanith/src/2d/g2dpolylinecurve.cpp   0.1.0.0   edited Jun 30 08:00
**
...
// get a key
GError GPolyLineCurve2D::Key(const GUInt32 Index, GPolyLineKey& KeyValue) const {
...
GError GPolyLineCurve2D::DoSetPointParameter(const GUInt32 Index, const GReal NewParamValue,
	...
	GPolyLineKey tmpKey(gKeys[Index]);
	...
	if (b) {
		...
		if (GMath::Abs(gKeys[i + 1].Parameter - NewParamValue) <= G_EPSILON) {
			...
			if (sameInterval) {
				...
				GDynArray<GPolyLineKey>::iterator it = gKeys.begin();
				it += (i + 1);
...
inline bool PolyLineKeyLE(const Amanith::GPolyLineKey& k1, const Amanith::GPolyLineKey& k2) {
...
GBool GPolyLineCurve2D::ParamToKeyIndex(const GReal Param, GUInt32& KeyIndex) const {
	...
	GDynArray<GPolyLineKey>::const_iterator result;
	GPolyLineKey tmpKey;
...
GError GPolyLineCurve2D::DoCut(const GReal u, GCurve2D *RightCurve, GCurve2D *LeftCurve) const {
	...
	if (lCurve) {
		...
		if (GMath::Abs(u - gKeys[keyIndex].Parameter) > G_EPSILON) {
			...
			GPolyLineKey tmpKey(u, GMath::Lerp(ratio, gKeys[keyIndex].Value, gKeys[keyIndex + 1].Value));
...
GError GPolyLineCurve2D::DoAddPoint(const GReal Parameter, const GPoint2 *NewPoint, GUInt32& Index,
	...
	if (Parameter < DomainStart() - G_EPSILON) {
		if (!NewPoint)
			return G_INVALID_PARAMETER;
		GPolyLineKey tmpKey(Parameter, *NewPoint);
		gKeys.insert(gKeys.begin(), tmpKey);
		Index = 0;
		AlreadyExists = G_FALSE;
		return G_NO_ERROR;
	...
	if (Parameter >= DomainEnd() - G_EPSILON) {
		...
		// in this case point must be added ON the curve
			...
			if (NewPoint) {
				...
				GPolyLineKey tmpKey(Parameter, GMath::Lerp(ratio, gKeys[keyIndex].Value, gKeys[keyIndex + 1].Value));
				it += (keyIndex + 1);
				gKeys.insert(it, tmpKey);
				Index = keyIndex + 1;
			}
